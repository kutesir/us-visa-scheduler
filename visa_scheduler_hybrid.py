...
def save_error_screenshot(prefix="error"):
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    os.makedirs("screenshots", exist_ok=True)
    filename = os.path.join("screenshots", f"{prefix}_{timestamp}.png")
    driver.save_screenshot(filename)
    logging.error(f"📸 Screenshot saved: {filename}")
...
def browser_login():
    ...
    Wait(driver, 60).until(EC.presence_of_element_located(
        (By.XPATH, f"//a[contains(text(), '{REGEX_CONTINUE}')]")))
...
def browser_get_time(date):
    ...
    if not available_times:
        logging.warning(f"No available times for {date}")
        return None
...
def browser_reschedule(date):
    ...
    if not time:
        msg = f"❌ No time available for {date}. Skipping."
        logging.warning(msg)
        send_telegram(msg)
        return "FAIL", msg
    ...
    if "sign_in" in driver.current_url:
        send_telegram("⚠️ Session expired. Redirected to login.")
        return "FAIL", "Redirected to login page"
    ...
    try:
        ...
        save_error_screenshot("reschedule_fail")
        logging.error(str(e))
        return "FAIL", f"Error during rescheduling: {e}"
...
def safe_browser_login():
    ...
    while attempts < 3:
        try:
            ...
            logging.warning(msg)
            send_telegram(msg)
            save_error_screenshot("login_fail")
            time.sleep(30)
...
def safe_browser_reschedule(date):
    ...
    while attempts < 3:
        try:
            ...
            logging.warning(msg)
            send_telegram(msg)
            save_error_screenshot("reschedule_retry_fail")
            time.sleep(30)
...
if __name__ == "__main__":
    ...
    # chrome_options.add_argument("--headless")  # Optional: for visible Chrome
    ...
    while retry_attempts < max_retries:
        ...
        try:
            ...
            logging.error(msg)
            send_telegram(msg)
            save_error_screenshot("unexpected")
            time.sleep(120)
